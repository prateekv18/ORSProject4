...
public class UserRegistrationCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(login)) {
			 ...
			 * PropertyReader.getValue("error.email", "Invalid ")); pass = false; }
			 */
		...
		if (DataValidator.isNull(request.getParameter("mobileNo"))) {
			 ...
			 * "Mobile No. contain 10 Digits & Series start with 6-9"); pass = false; }
			 */
		...
		if (DataValidator.isNull(dob)) {
			 ...
			 * else if (!DataValidator.isvalidateAge(dob)) { request.setAttribute("dob",
			 * PropertyReader.getValue("error.require", "Minimum Age 18 year")); pass =
			 * false; }
			 */
...
//     long id = DataUtility.getLong(request.getParameter("id"));
		...
		if (OP_SIGN_UP.equalsIgnoreCase(op)) {
			...
			try {
				 ...
				 * catch(RecordNotFoundException re) { re.printStackTrace(); }
