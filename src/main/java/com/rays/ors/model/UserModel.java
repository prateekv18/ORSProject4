...
import com.rays.ors.util.EmailBuilder;
import com.rays.ors.util.EmailMessage;
import com.rays.ors.util.EmailUtility;
	...
	public void update(UserBean bean) throws ApplicationException, DuplicateRecordException, RecordNotFoundException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE ST_USER SET FIRST_NAME=?,LAST_NAME=?,LOGIN_ID=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,UNSUCCESSFUL_LOGIN=?,GENDER=?,LAST_LOGIN=?,USER_LOCK=?,REGISTERED_IP=?,LAST_LOGIN_IP=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getLogin() != null && bean.getLogin().length() > 0) {
				sql.append(" AND LOGIN_ID like '" + bean.getLogin() + "%'");
			}
			if (bean.getRoleId() > 0) {
...
// Calculate start record index
	...
	public long registerUser(UserBean bean)
			throws DuplicateRecordException, RecordNotFoundException, ApplicationException {
	 ...
	 * } } catch (DuplicateRecordException e) { log.error("Application Exception..",
	 * e); throw new ApplicationException("Database Exception"); }
